...
- **c4-verifier** (Haiku) — independent verification across three modes (`coverage`, `drift`, `iac`).
...
- `containers/c4-container-<name>.md` — per-container detail
...
| `/banyan-c4` | C4 architecture documentation | Recommended after /banyan-init on large brownfield codebases |
...
- When running commands that are slow (>30s), expensive, or produce diagnostic output, **always `tee` the full output to a log file**.
- Use `.claude-logs/` at the project root for log files. Create the directory if it doesn't exist.
...
- `memory-bank/learning-metrics.md` - Learning configuration and effectiveness tracking
